...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        lstm=BiLSTM_Attention(config,wordEmbedding)
        ...
        savedModelPath = config.preconfig.modelPath_BiLSTM_Attention
        ...
        def trainStep(batchX,batchY):
            feed_dict={
                lstm.inputX: batchX,
                lstm.inputY: batchY,
                lstm.dropKeepprob: config.model.dropkeepProb
            }
            _, summary, step, loss, predictions=sess.run([trainOp,summaryOp,globalStep,lstm.loss,lstm.prediction],feed_dict=feed_dict)
        ...
        def devStep(batchX,batchY):
            feed_dict={
                lstm.inputX:batchX,
                lstm.inputY:batchY,
                lstm.dropKeepprob:1.0
            }

            summary,step,loss,predictions=sess.run([summaryOp,globalStep,lstm.loss,lstm.prediction],feed_dict=feed_dict)
