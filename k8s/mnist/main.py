...
def main():
    ...
    torch.save(model,"../data/saved_models_1.pth")
...
def hw():
    ...
    model=torch.load("../data/saved_models_1.pth")
    ...
    if flask.request.method == "POST":
        if flask.request.files.get("image"):
            ...
            if torch.cuda.is_available():
                a=outputs.data.cpu().numpy()[0].tolist()
            else:
                a=outputs.data.numpy()[0].tolist()
            a=a.index(max(a))
            data["predictions"] = []
            data["predictions"].append(a)
            data["success"] = True
